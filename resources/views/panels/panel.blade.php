        ...
        <div class="relative bg-white/5 backdrop-blur-xl border border-white/10 w-full max-w-2xl p-8 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
            ...
            <form action="{{ route('panels.store') }}" method="POST" class="space-y-6">
                ...
                <div class="pt-6 flex justify-end gap-4 border-t border-white/5">
                    ...
                    <button type="submit"
                        class="bg-[#FBB108] text-black font-bold py-3 px-10 rounded-xl shadow-lg">
                        Enregistrer
                    </button>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="group relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2.5rem] hover:border-[#FBB108]/50 transition-all duration-500 overflow-hidden flex flex-col">
            ...
            <div class="relative h-48 w-full overflow-hidden">
                ...
                <img src="{{ asset('images/panel.jpg') }}"
...
<div class="mt-auto pt-5 border-t border-white/5">
    ...
    <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-1 bg-black/30 px-3 py-1 rounded-full border border-white/5">
            ...
            <img src="http://openweathermap.org/img/wn/{{ $cityWeather['weather'][0]['icon'] }}.png"
                class="w-8 h-8"
                alt="weather">
    ...
    <div class="grid grid-cols-2 gap-2 text-[10px] text-gray-500 uppercase font-bold tracking-tight">
        ...
        <div class="flex items-center gap-1">
            ...
            <span>
                Vent:
                <span class="text-white">
                    {{ round($cityWeather['wind']['speed']) }} km/h
                </span>
            </span>
    ...
    @endif
                ...
                <div class="mt-auto pt-5 border-t border-white/5">
                    ...
                    <div class="flex gap-2 justify-end">
                        ...
                        {{-- DELETE --}}
                        <form action="{{ route('panels.destroy', $panel->id) }}"
                            method="POST"
                            class="inline">
...
{{-- EDIT MODAL --}}
<div id="editPanelModal"
    class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
    ...
    <div class="relative w-full max-w-lg overflow-hidden rounded-3xl border border-white/10 bg-[#0b0b0b]/95 shadow-[0_0_60px_rgba(251,177,8,0.08)]">
        ...
        <div class="flex items-center justify-between px-7 py-5 border-b border-white/5">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">
                        Mise à jour des informations du panneau solaire
                    </p>
                </div>
            ...
            <button onclick="closeEditModal()"
                class="w-10 h-10 rounded-xl bg-white/5 hover:bg-red-500 hover:text-white text-gray-400 transition-all flex items-center justify-center">
                ...
                <div class="space-y-2">
                    ...
                    <select id="edit_status"
                        ...
                        class="w-full py-3 px-4 bg-gray-600 rounded-2xl bg-white/5 border border-white/10 text-white outline-none focus:border-[#FBB108] transition-all">
                           ...
                           <option value="active" class="bg-[#111111] text-white">
         Active
    </option>

    <option value="inactive" class="bg-[#111111] text-white">
         Inactive
    </option>

    <option value="maintenance" class="bg-[#111111] text-white">
         Maintenance
    </option>
...
<div class="grid grid-cols-2 gap-4">
    ...
    <div class="space-y-2">
        ...
        <div class="relative">
            ...
            <select id="edit_region"
                class="w-full pl-12 pr-4 py-3 bg-gray-600 rounded-2xl bg-white/5 border border-white/10 text-white outline-none focus:border-[#FBB108] transition-all">

                <option value=""  class="bg-yellow-500 font-bold" >Choisir région</option>

                @foreach($zones->unique('name') as $zone)
                    <option value="{{ $zone->name }}"  class="bg-[#111111] text-white">
                        {{ $zone->name }}
                    </option>
                @endforeach
    ...
    <div class="space-y-2">
        ...
        <div class="relative">
            ...
            <select id="edit_city"
                name="zone_id"
                class="w-full pl-12 pr-4 py-3 rounded-2xl bg-white/5 border border-white/10 text-white outline-none focus:border-[#FBB108] transition-all">

                <option value="" class="bg-[#111111] text-white">Choisir ville</option>
            ...
            <div class="flex items-center justify-end gap-3 pt-5 border-t border-white/5">
                ...
                <button type="button"
                    ...
                    class="px-5 py-3 rounded-2xl bg-white/5 hover:bg-white/10 text-gray-300 transition-all">
                ...
                <button type="submit"
                    class="px-6 py-3 rounded-2xl bg-[#FBB108] hover:bg-[#ffd04d] text-black font-bold shadow-[0_0_20px_rgba(251,177,8,0.25)] transition-all active:scale-95 flex items-center gap-2">
    ...
    if (regionSelect) {
        ...
        regionSelect.addEventListener('change', function () {
            ...
            const filteredCities =
                allZones.filter(zone => zone.name === selectedRegion);
            ...
            filteredCities.forEach(zone => {
                ...
                // Modification: Zidna background o color
...
function openEditModal(id, name, serial, power, status, zoneId) {
    ...
    document.getElementById('editPanelForm').action =
        `/panels/${id}`;
    ...
    if (currentZone) {
        ...
        const filteredCities =
            allZones.filter(zone => zone.name === currentZone.name);

        editCity.innerHTML = '<option value="" class="bg-yellow-500 text-white font-bold">Choisir ville</option>';
        ...
        filteredCities.forEach(zone => {
            ...
            option.value = zone.id;
            option.textContent = zone.city;
            // Modification: Zidna background o color hna tani
            option.classList.add('bg-[#111111]', 'text-white');
